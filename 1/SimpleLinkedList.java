...
public class SimpleLinkedList<T> implements Iterable<T> {
	...
	public int indexOf(Object o){
		...
		while(itr1.hasNext()){
			if(itr1.current.data.equals(desiredO)){
				return itr1.index();
			}
			else
				itr1.next();
	 ...
	 * 
	 * Answer: To make reverse operate in a single timestep you would need to switch the node links for each element.
	 */
	public void reverse(){
		LinkedListIterator itr2= this.iterator();
		
		while(itr2.hasPrev() && itr2.current!=endMarker) {
			System.out.println("entered while loop...");
			
			this.add(itr2.current.data);
			System.out.println("Added current to end of the list");
			System.out.println(this);
			if (itr2.hasPrev()){
				System.out.println("moving to previous object in list");
				itr2.prev();
			}
		
			itr2.remove(); 
			System.out.println("Removed old current");
			System.out.println(itr2.hasPrev());
			System.out.println(itr2.current!=endMarker);
			
		}
	}
	/**
	 * It would take O((n-1)+(n-2)+(n-3)...(n-n)) or the arithmetic sum of N
	 * 
	 * And to optimize, you'd have to have the list sorted.
	 */
	public void removeDuplicates(){
		...
		while(outsideItr.hasNext()){
			...
			while(insideItr.hasNext()){
				...
				if (!foundDuplicate && insideItr.hasNext()) {
					insIdx++;
					insideItr.next();
	...
	private class LinkedListIterator implements java.util.Iterator<T> {
		...
		public T prev(){
			...
			System.out.println("current should be " + current.prev.data);
			current = current.prev;
			itrIndex--; //decrements index to represent change in current node
			
			okToRemovePrev = true;
			System.out.println("it's okay to removeprev");
			return prevItem;
		}
		 ...
		 */
		public void remove() {
			if (!okToRemoveNext && !okToRemovePrev)
				throw new IllegalStateException();
			
			if(okToRemovePrev){
				System.out.println("It's okay to remove");
				SimpleLinkedList.this.remove(current.next);
			}
			if(okToRemoveNext){
